...
#################update 1 se######################
from anomaly_detection.dpa.Attn_models import SEAttention
...
class Resnet9EncoderNetworks(AbstactEncoderNetworks):
    ...
    def _get_intermediate_block(self, resolution):
        ...
        layers = [
            self._get_block(PreActResnetBlockDown, prev_nf, nf),  SEAttention(nf),  
        ] #%%%%%%%%%%%%%%%%%%============SEAttention(nf),Adding se block==============================%%%%%%%%%%%%%%
    ...
    def _get_rgb_block(self, resolution):
        ...
        layers = [
            ...
            self._get_block(PreActResnetBlock, nf, nf),  SEAttention(nf),  
        ]#%%%%%%%%%%%%%55===================SEAttention(nf),===========================%%%%%%%%%%%%%%%%
        return self._init_layers(nn.Sequential(*layers))
