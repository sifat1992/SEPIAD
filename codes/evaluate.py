...
def evaluate(config):
    ...
    # Save the raw anomaly scores and labels for plotting ROC curve### new
    raw_output_path = os.path.join(results_root, 'roc_data.csv')
    df = pd.DataFrame({
    "label": y_true,
    "score": y_pred
    })
    df.to_csv(raw_output_path, index=False)
    print(f"Saved ROC data to: {raw_output_path}")



def predict_anomaly_scores(gen, enc, image_rec_loss, dataset, batch_size):
    data_loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False, num_workers=1)
